...
def process_transaction(tx_hash, tag='noble'):
    ...
    input_data = tx.input
    address_hex_str, nonce = decode_function_input(contract, input_data)
    if address_hex_str == 'Error decoding input':
        return 'Error decoding transaction input', 'Error decoding nonce'
    else:
        bech32_address = address_to_bech32(address_hex_str, tag)
        return bech32_address, nonce
...
def decode_function_input(contract, input_data):
    ...
    try:
        ...
        message_hex = (func_params['message']).hex()
        last_40_chars = message_hex[-40:]
        nonce_included_hex = message_hex[24:40]  # Hexadecimal string of the nonce
        nonce_included = int(nonce_included_hex, 16)  # Convert hex to int
        return last_40_chars, nonce_included
...
def process_tx_hashes_and_update_csv(file_path):
    ...
    with open(file_path, mode='r', newline='') as file:
        ...
        header = next(reader)
        updated_rows.append(header + ['Noble Address', 'Dydx Address', 'Nonce'])
        for row in reader:
            tx_hash = row[0].strip()
            noble_address, nonce = process_transaction(tx_hash)
            dydx_address = noble_to_dydx(noble_address)
            updated_rows.append(row + [noble_address, dydx_address, nonce])
